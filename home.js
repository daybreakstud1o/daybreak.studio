function setupCaseStudyTransition(number) {
    ...
    var asdfhasdfhjkasfd = event.target.getAttribute("id");
	...
	transitionLinkElms.forEach((item) => {
		item.addEventListener ('click', handleProjectLinkClick)
	})
	
	function cleanupClickListeners() {
		transitionLinkElms.forEach((item) => item.removeEventListener("click", handleProjectLinkClick))
	}
...
router.useScript(()=>{
	...
	infinite();
	function cleanupInfiniteScrollListener() {
		window.removeEventListener("resize", updateInfiniteScroll);
		window.removeEventListener("scroll", updateInfiniteScroll);
	}
	function updateInfiniteScroll() {
		infinite();
	}
	window.addEventListener("resize", updateInfiniteScroll);
	window.addEventListener("scroll", updateInfiniteScroll);
